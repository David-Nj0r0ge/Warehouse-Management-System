...
if(isset($_POST['reg-no']))
{
  //$product_code ="#" . substr(rand(),0,4);
    ...
    $stmt2->bindParam(":reg",$reg_no);
    $stmt2->execute();
    $row2 = $stmt2->fetch(PDO::FETCH_ASSOC);
    if($stmt2->rowCount()>0)
    {
$stmt3 = $DB_con->prepare("SELECT * FROM student_details WHERE reg_no = :reg_no");
$stmt3->bindParam(":reg_no",$row2['found_by']);
$stmt3->execute();
...
<?php
    ...
    else
    {
      ...
      </span> &nbsp; ID Not Found Check Out Later</div>
      <?php
    }
    if(empty($reg_no ))
    {
...
<?php
    ...
    else
    {
      ...
      <?php
             ...
             $stmt4=$DB_con->prepare("INSERT INTO losts(reg_no)VALUES(:reg)");
    $stmt4->bindParam(":reg",$reg_no);
      ...
      $stmt4->execute();
        ...
        <?php
        echo "Error Adding Lost ID Try Again Please";
        ?>
    </div>
                         <?php
                    }
