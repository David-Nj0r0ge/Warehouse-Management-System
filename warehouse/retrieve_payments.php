...
$stmt5->execute();
	$stmt8=$DB_con->prepare("SELECT container_space_bookings.*, received_conts.*,users_content.*,payments.* FROM container_space_bookings JOIN received_conts ON container_space_bookings.book_id=received_conts.cont_id JOIN
						users_content ON users_content.email=received_conts.owner JOIN payments ON payments.cont_cod=received_conts.cont_id WHERE users_content.email=:em");
	$stmt8->bindParam(":em",$user_id);
	$stmt8->execute();
...
$charge_by_no_days= $rw5['cost'];
	if($stmt8->rowCount()<1)
	{
		...
		<div class="w3-card-8 w3-padding-16 w3-red">
			<h1 class="w3-text-white w3-center w3-xlarge">You Haven't Made Any Payments Yet</h1>
		</div>
		<?php
	}
	else
	{
		?>
    <span id="owned" style="display: none;"><i><span class="w3-text-blue">Name &nbsp:<span class="w3-allerta w3-xlarge w3-text-black"><?php echo $results['first_name'] . " " . $results['last_name']?></span></i></span>
